  ...
  (let ((ht (make-hash-table :test 'equal)))
    (setf (gethash "type" ht)
          (ecase (ir-type-primitive-kind ir-type)
            ...
            (:number "number")
            (:boolean "boolean")))
    (macrolet ((emit-when (accessor key)
                 `(let ((v (,accessor ir-type)))
                    (when v (setf (gethash ,key ht) v)))))
      (emit-when ir-type-primitive-minimum "minimum")
      (emit-when ir-type-primitive-maximum "maximum")
      (emit-when ir-type-primitive-exclusive-minimum "exclusiveMinimum")
      (emit-when ir-type-primitive-exclusive-maximum "exclusiveMaximum")
      (emit-when ir-type-primitive-min-length "minLength")
      (emit-when ir-type-primitive-max-length "maxLength")
      (emit-when ir-type-primitive-pattern "pattern"))
    ht))
...
(defun %list-to-json-schema (ir-type in-progress defs)
  (let ((ht (make-hash-table :test 'equal)))
    ...
    (let ((mi (ir-type-list-min-items ir-type)))
      (when mi (setf (gethash "minItems" ht) mi)))
    (let ((ma (ir-type-list-max-items ir-type)))
      (when ma (setf (gethash "maxItems" ht) ma)))
    ht))
